...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex flex-col w-60 shrink-0 p-4 space-y-2 bg-slate-900 border-r border-slate-800/80 min-h-[calc(100vh-4rem)] justify-between">
        <div>
        ...
        {/* Sidebar Footer Install Box */}
        {!isStandalone && onOpenInstallPWA && (
          <div className="pt-3 border-t border-slate-800/80">
            <button
              onClick={onOpenInstallPWA}
              className="w-full py-2 px-3 rounded-xl bg-indigo-950/40 border border-indigo-800/60 hover:bg-indigo-900/50 text-indigo-300 hover:text-white text-xs font-semibold flex items-center justify-center gap-2 transition-all"
            >
              <Download className="w-3.5 h-3.5 text-indigo-400" />
              <span>Install Friend OS PWA</span>
            </button>
          </div>
        )}
      </aside>
      ...
      <div 
        ...
        className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-slate-950/95 backdrop-blur-xl border-t border-slate-800/90 px-3 pt-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] flex items-center justify-around shadow-2xl"
      >
        {/* 1. Home */}
        <button
